...
def one_adaptation(net, raw_image, probs, counts, images, config, device='cpu'):
    ...
    ##
    warped = warp_perspective(raw_image, M, dsize=(H,W), align_corners=True)
    mask = warp_perspective(torch.ones([B,1,H,W], device=device), M, dsize=(H, W), mode='nearest',align_corners=True)
    count = warp_perspective(torch.ones([B,1,H,W],device=device), M_inv, dsize=(H,W), mode='nearest',align_corners=True)
    ...
    prob = prob['prob']
    prob = prob * mask
    prob_proj = warp_perspective(prob.unsqueeze(dim=1), M_inv, dsize=(H,W), align_corners=True)
